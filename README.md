...
```
packages/core   @agent-chat/core   零 UI：类型 / store / transport / hooks
packages/a2ui   @agent-chat/a2ui   A2UI 协议类型 + 渲染器
packages/ui     @agent-chat/ui     组件 + design token + 默认 A2UI 组件集
...
```bash
pnpm add @agent-chat/ui @agent-chat/core @agent-chat/a2ui
```
...
/* app.css */
@import 'tailwindcss';
@import '@agent-chat/ui/tokens.css';

/* 让 Tailwind 扫到组件里的类名，避免重复生成一份 */
@source '../node_modules/@agent-chat/ui/dist';
```

没用 Tailwind：

```ts
import '@agent-chat/ui/style.css'
```
...
import { createSSETransport, useChat } from '@agent-chat/core'
import {
  ChatContainer,
  ChatMessageList,
  ChatThemeProvider,
...
```tsx
import { useAttachments, useVoiceInput } from '@agent-chat/core'
